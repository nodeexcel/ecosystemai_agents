...
def create_a_email_campaign(payload: EmailCampaignCreation, db: Session = Depends(get_db),
    ...
    db.commit()
    return JSONResponse(content={'success': _("Email Campaign is created successfullly.")}, status_code=201)

@router.get("/get-email-campaigns")
def get_all_campaigns(db: Session = Depends(get_db), user_id: str = Depends(get_current_user), _ = Depends(get_translator_dependency)):
...
def update_email_campign(campaign_id, payload: UpdateEmailCampaign,  db: Session = Depends(get_db),
                         user_id: str = Depends(get_current_user), _ = Depends(get_translator_dependency)):
...
def status_change_of_campaign(campaign_id,  db: Session = Depends(get_db),
                              user_id: str = Depends(get_current_user), _ = Depends(get_translator_dependency)):
    ...
    if campaign.status != "draft" or campaign.status != "issue_detected":
        if campaign.is_active == False:
            ...
            db.commit()
            return JSONResponse(content={'success': _('status updated for campaign')}, status_code=200)
...
def get_campaign_schedule(db: Session = Depends(get_db), user_id: str = Depends(get_current_user), _ = Depends(get_translator_dependency)):
    ...
    campaigns = db.query(EmailCampaign).filter_by(user_id=user_id).all()
    if not campaigns:
        return JSONResponse(content={"Success": _("You donot have any ongoing campaigns")}, status_code=200)
...
def get_scheduled_content(content_id, db: Session = Depends(get_db),
                          user_id: str = Depends(get_current_user), _ = Depends(get_translator_dependency)):
    ...
    return JSONResponse(content={"error": _("No content exits")}, status_code=404)


@router.post("/content-status/{content_id}")
def get_scheduled_content(content_id, payload: ContentCheck, db: Session = Depends(get_db),
                          user_id: str = Depends(get_current_user), _ = Depends(get_translator_dependency)):
    ...
    if content and content.status != "planned":
        ...
        if payload.status == "pending_approval":
            ...
            db.commit()
            return JSONResponse(content="", status_code=200)
    return JSONResponse(content={"error": _("Cannot edit the content")}, status_code=400)

@router.post("/duplicate-campaign/{campaign_id}")
def duplicate_a_campaign(campaign_id, db: Session = Depends(get_db),
                         user_id: str = Depends(get_current_user), _ = Depends(get_translator_dependency)):
